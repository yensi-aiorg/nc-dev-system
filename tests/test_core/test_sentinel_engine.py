...
from ncdev.core.models import SentinelPhase, SentinelTaskStatus
...
class TestRunSentinelFixDryRun:
    ...
    def test_load_report_task_passed(self, tmp_path: Path):
        ...
        assert load_task.status == SentinelTaskStatus.PASSED
    ...
    def test_other_tasks_stay_pending(self, tmp_path: Path):
        ...
        for task in state.tasks:
            if task.name in other_names:
                assert task.status == SentinelTaskStatus.PENDING, f"{task.name} should be pending"
...
class TestRunSentinelFixInvalidReport:
    def test_invalid_json_blocks(self, tmp_path: Path):
        ...
        assert state.phase == SentinelPhase.BLOCKED
        load_task = next(t for t in state.tasks if t.name == "load_report")
        assert load_task.status == SentinelTaskStatus.BLOCKED
...
class TestRunSentinelFixMissingReport:
