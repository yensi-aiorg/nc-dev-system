"""Tests for the `mode` switch + MODE_PRESETS in core config."""
...
def test_mode_roundtrip_via_yaml(tmp_path: Path):
    cfg_path = tmp_path / ".nc-dev" / "config.yaml"
    cfg_path.parent.mkdir(parents=True, exist_ok=True)
    NCDevConfig(mode="codex_only").to_yaml_dict()
    raw = NCDevConfig(mode="codex_only").to_yaml_dict()
