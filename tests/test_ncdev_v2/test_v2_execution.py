...
def test_execute_routed_tasks_records_stubbed_results(tmp_path: Path) -> None:
    ...
    routing = RoutingPlanDoc(
        ...
        source_inputs=[],
        decisions=[
            RoutingDecision(task_type=TaskType.SOURCE_INGEST, provider="openai_codex", model="gpt-5.4", rationale="test"),
            RoutingDecision(task_type=TaskType.MARKET_RESEARCH, provider="openai_codex", model="gpt-5.4", rationale="test"),
            RoutingDecision(task_type=TaskType.FEATURE_EXTRACTION, provider="openai_codex", model="gpt-5.4", rationale="test"),
            RoutingDecision(task_type=TaskType.DESIGN_BRIEF, provider="openai_codex", model="gpt-5.4", rationale="test"),
