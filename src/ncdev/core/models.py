...
class SentinelTaskStatus(str, Enum):
    PENDING = "pending"
    RUNNING = "running"
    PASSED = "passed"
    FAILED = "failed"
...
class TargetProjectContractDoc(ArtifactEnvelope):
    schema_id: str = "target-project-contract.1"
    project_name: str
    target_type: str
    operating_mode: str = "website_saas"
    target_repo_path: str = ""
...
class ScaffoldManifestDoc(ArtifactEnvelope):
    schema_id: str = "scaffold-manifest.1"
    project_name: str
    target_path: str
...
class VerificationContractDoc(ArtifactEnvelope):
    schema_id: str = "verification-contract.1"
    project_name: str
    commands: list[str] = Field(default_factory=list)
...
class CapabilitySnapshotDoc(ArtifactEnvelope):
    schema_id: str = "capability-snapshot.1"
...
class RoutingPlanDoc(ArtifactEnvelope):
    schema_id: str = "routing-plan.1"
    decisions: list[RoutingDecision] = Field(default_factory=list)


class TaskRequestDoc(ArtifactEnvelope):
    schema_id: str = "task-request.1"
    task_type: TaskType
    provider: str
    model: str
    title: str
...
class EvidenceIndexDoc(ArtifactEnvelope):
    schema_id: str = "evidence-index.1"
    project_name: str
    target_path: str
...
class VerificationIssueBundleDoc(ArtifactEnvelope):
    schema_id: str = "verification-issues.1"
    project_name: str
    target_path: str
    issue_count: int = 0
...
class DeliverySummaryDoc(ArtifactEnvelope):
    schema_id: str = "delivery-summary.1"
    project_name: str
    target_type: str
    stack: dict[str, str] = Field(default_factory=dict)
    batch_count: int
...
class SentinelTaskState(BaseModel):
    name: str
    status: SentinelTaskStatus = SentinelTaskStatus.PENDING
    message: str = ""
    artifacts: list[str] = Field(default_factory=list)


class SentinelRunState(BaseModel):
    run_id: str
    command: str
    workspace: str
    run_dir: str
    phase: SentinelPhase = SentinelPhase.INIT
    status: SentinelTaskStatus = SentinelTaskStatus.RUNNING
    started_at: datetime = Field(default_factory=_utc_now)
    updated_at: datetime = Field(default_factory=_utc_now)
    tasks: list[SentinelTaskState] = Field(default_factory=list)
    artifacts: list[str] = Field(default_factory=list)
...
class FullRunReportDoc(ArtifactEnvelope):
    schema_id: str = "full-run-report.1"
    run_id: str
    command: str
    project_name: str = ""
    target_path: str = ""
